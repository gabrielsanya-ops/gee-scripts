...
// Apply filter where country name equals Saudi.
var saudi = dataset.filter(ee.Filter.eq('country_na', 'Saudi Arabia'));
var bangalore = ee.FeatureCollection('users/ujavalgandhi/public/bangalore_boundary');

var s2 = ee.ImageCollection('COPERNICUS/S2_SR');

// The following collections were created using the Drawing Tools in the code editor
...
var filtered = s2
  ...
  .filter(ee.Filter.bounds(saudi))
  .select('B.*');

var composite = filtered.median().clip(saudi);
...
// Display the GCPs - use the style() function to style the GCPs
...
var gcpsStyled = ee.FeatureCollection(
  landcover.map(function(lc) {
    var idx = landcover.indexOf(lc);
    var color = palette.get(idx);
